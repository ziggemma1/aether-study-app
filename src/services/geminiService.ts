  ...
  try {
    ...
    for (const model of models) {
      try {
        ...
        lastError = error;
        console.warn(`Model ${model} failed for visual aid:`, error);
      }
    }
    return '';
  } catch (error) {
...
export const generateGeminiTopicSection = async (content: string, title: string, topic: string, language: string = "English (US)"): Promise<NoteSection> => {
  ...
  for (const model of models) {
    try {
      ...
      lastError = error;
      console.warn(`Model ${model} failed for Gemini topic section:`, error);
    }
  }
  throw lastError || new Error("All Gemini models failed for topic section");
};
    ...
    try {
      ...
      for (const model of models) {
        try {
          ...
          lastError = error;
          console.warn(`Model ${model} failed for analysis stage 1, trying next...`);
        }
      }

      if (!text) throw lastError || new Error("Gemini initial analysis failed");
  ...
  try {
    ...
    for (const model of models) {
      try {
        ...
        lastError = error;
        console.warn(`Model ${model} failed for study plan, trying next...`);
      }
    }

    if (!text) throw lastError || new Error("Gemini study plan generation failed");
    return JSON.parse(text);
  } catch (error: any) {
