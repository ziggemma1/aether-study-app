        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6">
          {user?.plan === 'pro' && (
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="glass-card p-6 border-[#00D2FF]/20 bg-[#00D2FF]/5"
            >
              <div className="flex flex-col items-center text-center space-y-4">
                <div className="w-16 h-16 bg-[#00D2FF]/10 rounded-full flex items-center justify-center border border-[#00D2FF]/30 shadow-[0_0_20px_rgba(0,210,255,0.2)]">
                  <Infinity size={32} className="text-[#00D2FF]" />
                </div>
                <div className="space-y-1">
                  <h3 className="text-sm font-black text-[#F0F3F8] uppercase tracking-widest">Neural Link Stabilized</h3>
                  <p className="text-[10px] text-[#8E9AAF] uppercase tracking-widest opacity-60">Infinite Bandwidth Active</p>
                </div>
              </div>
            </motion.div>
          )}
          ...
          >
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Neural Economy</span>
              <div className="w-1.5 h-1.5 bg-[#00D2FF] rounded-full animate-pulse shadow-[0_0_8px_#00D2FF]" />
            </div>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <span className="text-[9px] font-black text-[#8E9AAF] uppercase tracking-widest block">Neural Bits</span>
                <span className="text-xl font-black text-[#F0F3F8] tracking-tighter tabular-nums">
                  {user?.points?.toLocaleString() || 0}
                </span>
              </div>
              <div className="space-y-1 border-l border-white/5 pl-4">
                <span className="text-[9px] font-black text-[#8E9AAF] uppercase tracking-widest block text-[#6C5CE7]">Aether Credits</span>
                <span className="text-xl font-black text-[#6C5CE7] tracking-tighter tabular-nums">
                  {user?.aetherPoints?.toLocaleString() || 0}
                </span>
              </div>
            </div>
          </motion.div>
