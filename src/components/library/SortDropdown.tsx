  ...
  return (
    <div ref={containerRef} className="relative w-full sm:w-auto">
      <button
        ...
        className="flex w-full sm:w-60 items-center justify-between gap-2.5 rounded-xl border border-border bg-surface px-4 py-3 text-sm font-semibold text-text-main shadow-sm transition-all active:scale-[0.98] min-h-[44px]"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 right-0 sm:left-auto sm:right-0 z-50 mt-2 rounded-2xl border border-border bg-surface p-1.5 shadow-2xl min-w-48"
          >
            {OPTIONS.map((opt) => (
              <button
                key={opt.value}
